...
$configs = @(
    @{
        Out='trimly-social-12-carousel-discovery-v1.jpg';
        Title='Discover the right barber faster.';
        Subtitle='Lead with search, ratings, service tags, and trusted first impressions.';
        Hero='trimly-social-03-browse-barbers.jpg';
        Inset='trimly-social-01-landing-hero.jpg';
        HeroTitle='Browse by location, service, and price';
        InsetTitle='Strong homepage first impression';
        Chips=@('Discovery','Ratings','NGN pricing')
    },
    @{
        Out='trimly-social-13-carousel-profile-v1.jpg';
        Title='Profile pages that build booking confidence.';
        Subtitle='Show services, availability, location, and clear reasons to trust the barber.';
        ...
        InsetTitle='Smooth onboarding into the flow';
        Chips=@('Availability','Services','Home service')
    },
    @{
        Out='trimly-social-14-carousel-payment-v1.jpg';
        Title='Approved bookings move cleanly into payment.';
        Subtitle='Status, trust indicators, and a clear amount due make the next step obvious.';
        ...
        InsetTitle='Chat covers last-mile coordination';
        Chips=@('Approved','Paid','Trust built in')
    },
    @{
        Out='trimly-social-15-carousel-dashboards-v1.jpg';
        Title='Dashboards for both sides of the marketplace.';
        Subtitle='Customers track bookings while barbers manage earnings, approvals, and active work.';
...
foreach($cfg in $configs){
    ...
    $g.DrawString($cfg.Title,$headlineFont,$hb,(New-Object System.Drawing.RectangleF 54,118,360,145))
    $g.DrawString($cfg.Subtitle,$subFont,$mb,(New-Object System.Drawing.RectangleF 56,265,340,90))

    $x=56
    foreach($chip in $cfg.Chips){ $x += (Chip $g $chip $x 366 $chipFont $(if($chip -like '*Paid*' -or $chip -like '*pricing*' -or $chip -like '*Trust*'){ $gold } else { $green }) $(if($chip -like '*Paid*' -or $chip -like '*pricing*' -or $chip -like '*Trust*'){ (C 247 239 221) } else { (C 231 239 233) })) + 10 }

    $cb=New-Object System.Drawing.SolidBrush($green); Fill-RR $g $cb 56 422 174 50 24; $cb.Dispose(); $ct=New-Object System.Drawing.SolidBrush((C 248 244 236)); $g.DrawString('Carousel concept',(New-Object System.Drawing.Font('Segoe UI Semibold',13,[System.Drawing.FontStyle]::Bold)),$ct,83,437); $ct.Dispose()
    ...
    Card $g 58 544 338 250 28
    Img $g (Join-Path $root $cfg.Inset) 76 562 302 160 20 0
    $g.DrawString($cfg.InsetTitle,$labelFont,$hb,82,732)
    $g.DrawString('This supporting view helps the audience connect the full journey at a glance.',$smallFont,$mb,(New-Object System.Drawing.RectangleF 82,754,285,42))
    ...
    $footerRect = New-Object System.Drawing.RectangleF(56,845,340,110)
    $g.DrawString('Trimly keeps the flow understandable: discover, evaluate, book, pay, and communicate with confidence.',$subFont,$mb,$footerRect)
