...
$ink=C 31 54 45; $green=C 46 106 79; $gold=C 201 169 107; $muted=C 107 98 87

$configs = @(
    @{
        Out='trimly-social-16-carousel-landing-v1.jpg';
        Title='A homepage that explains Trimly fast.';
        Subtitle='The first screen should tell people what Trimly does, why it matters, and where to start.';
        Source='trimly-social-01-landing-hero.jpg';
        Mode='landscape';
        Footer='Strong launch cover for introducing the product.'
    },
    @{
        Out='trimly-social-17-carousel-browse-v1.jpg';
        Title='Discovery should feel simple and useful.';
        Subtitle='Customers need to compare barbers by service, location, and price without friction.';
        Source='trimly-social-03-browse-barbers.jpg';
        Mode='landscape';
        Footer='This is the kind of page that makes people want to try the product.'
    },
    @{
        Out='trimly-social-18-carousel-profile-v1.jpg';
        Title='Profiles should make booking feel confident.';
        Subtitle='Show the barber, services, availability, and home service details in one calm view.';
        Source='trimly-social-08-barber-profile-ngn-v1.jpg';
        Mode='portrait';
        Footer='A strong profile page reduces hesitation before booking.'
    },
    @{
        Out='trimly-social-19-carousel-payment-v1.jpg';
        Title='Approved bookings should move cleanly into payment.';
        Subtitle='The status, total amount, and trust indicators should all be easy to understand at a glance.';
        Source='trimly-social-09-booking-payment-ngn-v2.jpg';
        Mode='portrait';
        Footer='This slide explains how Trimly handles payment readiness.'
    },
    @{
        Out='trimly-social-20-carousel-chat-v1.jpg';
        Title='Chat should solve real home-service logistics.';
        Subtitle='Customers can share clear directions so the barber arrives smoothly for home service.';
        Source='trimly-social-10-chat-ngn-v2.jpg';
        Mode='portrait';
        Footer='Useful, human communication is part of the product experience.'
    }
)
...
foreach($cfg in $configs){
    ...
    $pageFont=New-Object System.Drawing.Font('Segoe UI Semibold',12,[System.Drawing.FontStyle]::Bold)

    Brand $g $ink $green $brandFont
    $pageBrush=New-Object System.Drawing.SolidBrush($gold)
    $g.DrawString('Trimly launch carousel',$pageFont,$pageBrush,56,110)
    $pageBrush.Dispose()

    $hb=New-Object System.Drawing.SolidBrush($ink)
    $mb=New-Object System.Drawing.SolidBrush($muted)
    $g.DrawString($cfg.Title,$headlineFont,$hb,(New-Object System.Drawing.RectangleF 56,138,700,96))
    $g.DrawString($cfg.Subtitle,$subFont,$mb,(New-Object System.Drawing.RectangleF 58,242,720,72))

    if($cfg.Mode -eq 'landscape'){
        Card $g 52 338 976 760 34
        ImgContain $g (Join-Path $root $cfg.Source) 80 366 920 704 26
        $g.DrawString($cfg.Footer,$footerFont,$mb,(New-Object System.Drawing.RectangleF 76,1118,920,48))
    } else {
        Card $g 170 338 740 860 34
        ImgContain $g (Join-Path $root $cfg.Source) 202 370 676 796 26
        $g.DrawString($cfg.Footer,$footerFont,$mb,(New-Object System.Drawing.RectangleF 172,1218,740,48))
    }

    $hb.Dispose(); $mb.Dispose(); $brandFont.Dispose(); $headlineFont.Dispose(); $subFont.Dispose(); $footerFont.Dispose(); $pageFont.Dispose(); $g.Dispose()
